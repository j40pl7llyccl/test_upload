...
namespace uIP.MacroProvider.StreamIO.VideoInToFrame
{
    public partial class FormConfVideoToFrame : Form
    {
        ...
        private void btnExtract_Click(object sender, EventArgs e)
        {
            ...
            // 指定輸出資料夾，例如在選取的資料夾下建立一個 "Output" 資料夾
            ...
            // 為避免 UI 卡住，使用 Task.Run 將影片處理移至背景執行緒中
            Task.Run(() =>
            {
                try
                {
                    // 呼叫 plugin 的 ProcessVideos 方法
                    videoExtractor.ProcessVideos(textBoxPickedDir.Text, outputFolder, intervalSeconds);
                ...
                catch (Exception ex)
                {
                    this.Invoke(new Action(() =>
                    {
                        MessageBox.Show($"影片處理發生錯誤：{ex.Message}", "錯誤", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }));
                }
            });
        }--*/

        //原本的btnExtract_Click的邏輯
        /*--
        public void ExtractVideos()
        {
            // 將 btnExtract_Click 的邏輯移動到這裡
            if (string.IsNullOrEmpty(textBoxPickedDir.Text) || !Directory.Exists(textBoxPickedDir.Text))
            {
                MessageBox.Show("請先選擇正確的影片資料夾！", "錯誤", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            ...
            Task.Run(() =>
            {
                try
                {
                    videoExtractor.ProcessVideos(textBoxPickedDir.Text, outputFolder, intervalSeconds, label_progress);
                    this.Invoke(new Action(() =>
                    {
                        MessageBox.Show("所有影片處理完成！", "完成", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }));
