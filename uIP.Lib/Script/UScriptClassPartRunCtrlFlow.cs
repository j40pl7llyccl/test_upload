...
namespace uIP.Lib.Script
{
    public partial class UScript
    {
                                           ...
                                           out List<UScriptHistoryCarrier> retHistCarriers)
        {
            ...
            fpUDataCarrierSetResHandler prevPropagationCarrierSetHandler = null;
            ...
            for (;;)
            {
                ...
                if ( bSyncCall )
                {
                    if ( !script.SynchronizedObject.WaitOne( nWaitTimeout ) )
                    {
                        ...
                        if ( enableLog && script.fpLog != null )
                            script.fpLog( eLogMessageType.NORMAL, 0, String.Format( "[UScript::RunningControlFlow] Exec {0} cannot sync a semaphore in {1}-ms", script.NameOfId, nWaitTimeout ) );
                        break;
                    }
                ...
                if ( script.Running( false, false, nIndexOfBegMacro, -1, prevPropagationCarrierSet, prevPropagationCarrierSetHandler, null, true, false ) != ScriptExecReturnCode.OK )
                    bGotErr = true;
                else
                {
