...
namespace uIP.Lib.UsrControl
{
    ...
    public class UsersData
    {
        ...
        virtual public void WriteGroup( string path )
        {
            // [available group]
            ...
            } catch { if ( m_fpLog != null ) m_fpLog(eLogMessageType.WARNING, 100, String.Format( "[{0}::WriteGroup] write path {1} error!", this.GetType().Name, String.IsNullOrEmpty(path) ? "" : path ) ); return; }

            if (m_bSettingFileMustEnc) FileEncryptUtility.ENC( path );
        }
        ...
        virtual public void WriteUsers( string path )
        {
            ...
            } catch { if ( m_fpLog != null ) m_fpLog(eLogMessageType.WARNING, 100, String.Format( "[{0}::WriteUsers] write path {1} error!", this.GetType().Name, String.IsNullOrEmpty( path ) ? "" : path ) ); return; }

            if ( m_bSettingFileMustEnc )
                FileEncryptUtility.ENC( path );
            if ( m_fpLog != null ) m_fpLog(eLogMessageType.NORMAL, 150, String.Format( "[{0}::WriteUsers] write file {1} success!", this.GetType().Name, path ) );
        }
