...
namespace uIP.Lib.Utility
{
    public static class CommonUtilities
    {
        static CommonUtilities()
        {
            ...
            {
                if ( names[ i ] == KnownColor.AliceBlue.ToString() )
                {
                    beginI = i; break;
                }
            }
            ...
            for ( int i = 0; i < names.Length; i++ )
            {
                if ( names[ i ] == KnownColor.YellowGreen.ToString() )
                {
                    endI = i; break;
                }
            }
            ...
            if ( beginI > 0 && endI > 0 )
            {
                ...
                {
                    if ( Enum.TryParse<KnownColor>( names[ i ], true, out var v ) )
                        ColorList.Add( Color.FromKnownColor( v ) );
                }
            }

        }

        public static List<Color> ColorList { get; private set; }
        ...
        public static string Dump01(Exception e)
        {
            if ( e == null ) return "";
        ...
        public static string Dump02(Exception e)
        {
            if ( e == null ) return "";
            ...
            foreach(var f in frames)
            {
                ...
                trace.Append( "File:" + f.GetFileName() );
                trace.Append( ", Method:" + f.GetMethod().Name );
                trace.Append( ", LineNo.:" + f.GetFileLineNumber() );
                trace.Append( " --->\n" );
            }
