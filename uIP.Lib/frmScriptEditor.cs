...
namespace uIP.Lib
{
    public partial class FrmScriptEditor : Form
    {
        ...
        private UScript _scriptInstance;

        // Define a delegate and event
        public delegate void ScriptStoppedEventHandler(bool m_flag, object macro_instance);
        ...
        {
            InitializeComponent();
            _scriptInstance = new UScript(); // 創建實例
        ...
        {
            if (!string.IsNullOrEmpty(_currentRunnerId))
            {
                try
                {
                    if (ResourceManager.Get(ResourceManager.ScriptService) is UScriptService scriptService && _scriptInstance != null)
                    {
                        // 輸出目前腳本運行狀態供除錯用
                        Console.WriteLine("Current UnderRunning: " + _scriptInstance.UnderRunning);

                        // 如果腳本不在運行中，代表可能已經執行完畢
                        if (!_scriptInstance.UnderRunning)
                        {
                            MessageBox.Show("腳本已經執行完畢，無法取消。", "通知", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            _currentRunnerId = null;
                            return;
                        }
                        // 若腳本正在運行，再嘗試取消
                        bool result = _scriptInstance.CancelRunning();
                        Console.WriteLine("CancelRunning result: " + result);
                    }
                    else
                    {
                        MessageBox.Show("Script service 或 script instance 不可用！",
            ...
            {
                MessageBox.Show("No script is currently running!",
                                "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }


        //UScriptRunnerFactory is  in uIP.Lib/Service/UScriptRunner.cs file. and  UScriptService is in uIP.Lib/Service/UScriptService.cs file.
        private void toolStripMenuItem_withFreeMacroProgation_Click(object sender, EventArgs e)
        {
            ...
            _currentRunnerId = f.NewRunner0(n.Text, WithFreeMacroPropation_ScriptDoneCall, ss.GetScript(n.Text));

        }

        // 全域變數，存取目前執行中的 runner ID 與 runner 實例
        private UScriptRunner _scriptRunner = null;
        ...
        private void toolStripMenuItem_runWithoutFreePropagation_Click(object sender, EventArgs e)
        {
            ...
            // 設定一個非常大的執行次數，延長腳本運行時間
            UInt64 execTimes = 10000000000;

            // 傳入完整參數，注意 nExecTimes 要設為 execTimes
            _currentRunnerId = f.NewRunner0(null, null, ss.GetScript(n.Text), null, false, 0, execTimes);
            /*--
             if (string.IsNullOrEmpty(_currentRunnerId))
             {
                 MessageBox.Show("無法啟動腳本。", "錯誤", MessageBoxButtons.OK, MessageBoxIcon.Error);
                 return;
             }

             // 從 Factory 取得 Runner 實例，並將其 RunningScript 存入 _scriptInstance
             UScriptRunner runner = f.GetRunner(_currentRunnerId);
             if (runner == null)
             {
                 MessageBox.Show("無法取得 Runner 實例。", "錯誤", MessageBoxButtons.OK, MessageBoxIcon.Error);
                 return;
             }
             _scriptInstance = runner.RunningScript; // _scriptInstance 為正在執行的 UScript

             MessageBox.Show("腳本已啟動，Runner ID: " + _currentRunnerId, "資訊", MessageBoxButtons.OK, MessageBoxIcon.Information)
             --*/
        }
        ...
        private void contextMenuStrip_scriptRightCkl_Opening(object sender, System.ComponentModel.CancelEventArgs e)
        {
